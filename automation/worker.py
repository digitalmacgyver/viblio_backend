...
#           All rights reserved
# To do:
# Fork a new pypthon script for each patched file, data logging, error handling, checking before deleting files
#
#------------------------------------------------------------------------------
...
viblio_server_url = 'http://staging.viblio.com/services/mediafile/create?$params'
...
# Create thumbnail and poster files
            os.system ('ffmpeg -ss 10 -i /mnt/uploaded_files/' + s3_directory + '-vframes 1 -f image2 
              -s 128x128 /mnt/uploaded_files/' + s3_directory + _thumbnail.jpg')
# Upload to S3
...
# Update Viblio server
            params = {'filename': media_filename, 'mimetype': 'video/' + mime_extension, 'size': media_filesize, 'location':'us', 'bucket_name': config.bucket_name,
