...
def call_build_video_summary( user_uuid, year, album_id ): 
    ...
    movies = orm.query( Media, MediaAssets, MediaAlbums ).filter( and_(
    ...
    if simple:
        print "./script/wsclient.pl --service services/mediafile/create_video_summary -- summary_type=moments audio_track=%s summary_style=classic order=oldest title='CLASSIC SIMPLE YIR: %s' images[]='%s'" % ( random.choice( songs ), album.title, ','.join( video_summary['images[]'] ) )
    else:
        #print "./script/wsclient.pl --service services/mediafile/create_video_summary -- summary_type=moments audio_track=%s summary_style=template-2 order=oldest title='CLASSIC YIR: %s' images[]='%s' moment_offsets[]=0,3 summary_options='{\"template\" : \"email/27-yirSummary.tt\", \"subject\" : \"Whee!\", \"distribute_clips\" : \"side_by_side\" }'" % ( random.choice( songs ), album.title, ','.join( video_summary['images[]'] ) )
        print "./script/wsclient.pl --service services/mediafile/create_video_summary -- summary_type=moments audio_track=%s summary_style=template-2 order=oldest title='5 SEC: %s' images[]='%s' moment_offsets[]=-2.5,2.5 summary_options='{\"template\" : \"email/27-yirSummary.tt\", \"subject\" : \"Whee!\", \"distribute_clips\" : \"side_by_side\" }'" % ( random.choice( songs ), album.title, ','.join( video_summary['images[]'] ) )
...
if __name__ == '__main__':
    ...
    parser.add_option( "-a", "--album",
                       dest="album",
                       help="The ID of the album to summarize." )
    ...
    if not ( options.email or options.user_uuid ):
        ...
        if options.year:
            call_build_video_summary( user_uuid, int( options.year ), int( options.album ) )
        else:
